...
def export_payment_plans(path, rows, title='下月付款计划'):
    ...
    headers = ['序号', '合同编号', '合同名称', '对方单位', '款项名称',
               '应付日期', '应付金额', '已付金额', '未付金额',
               '付款条件', '负责人', '备注']
    col_widths = [6, 18, 26, 24, 16, 14, 14, 14, 14, 36, 14, 24]
    ...
    if rows:
        ...
        ws.cell(row=summary_row, column=7, value=round(total_due, 2)).number_format = '#,##0.00'
        ws.cell(row=summary_row, column=8, value=round(total_paid, 2)).number_format = '#,##0.00'
        ws.cell(row=summary_row, column=9, value=round(total_due - total_paid, 2)).number_format = '#,##0.00'
    else:
...
def export_contracts(path, contracts, title='合同台账'):
    ...
    headers = ['序号', '合同编号', '合同名称', '对方单位', '金额',
               '签订日期', '负责人', '状态', '创建时间', '付款计划数']
    col_widths = [6, 18, 26, 24, 14, 14, 14, 12, 14, 14]
    ...
    if contracts:
        ws.cell(row=summary_row, column=1, value='合计').font = _HEADER_FONT
        ws.cell(row=summary_row, column=5, value=round(total_amount, 2)).number_format = '#,##0.00'
    else:
        ws.cell(row=summary_row, column=1, value='（无数据）').font = _HEADER_FONT
    for ci in range(1, len(headers) + 1):
